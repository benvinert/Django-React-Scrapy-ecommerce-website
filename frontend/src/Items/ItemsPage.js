...
export default function Shoes() {
  ...
  const serializeAllQuerySearchParams = (allParams) => {
    ...
    console.log(
      "CAT",
      parametersSearch.category,
      "STYLE",
      parametersSearch.style,
      "GENDER",
      parametersSearch.gender,
      "COLOR",
      parametersSearch.color
    );
  ...
  const showFilter = () => {
    setShowFilterMenu({
      visibility: !showFilterMenu.visibility,
      doSpaceGrid: !showFilterMenu.doSpaceGrid,
    });
  };
  const getItems = async () => {
    setLoading(true);
    let URL = `${SERVER_PATH}${ITEMS_URLS.GET_ALL_ITEMS}`;
    if (searchparam) {
  ...
  const currentPosts = showItems.Products.slice(
    indexOfFirstPost,
    indexOfLastPost
  );
  ...
  const handleChangeColor = (event) => {
    ...
    let afterFilter = Allitems.Products.filter((each) => {
      ...
      return (
        each.price <= filterState.maxPrice &&
        each.color.toLowerCase().includes(color.toLowerCase()) &&
        ifSizeExists
      );
    });
  ...
  const handleChangePrice = (event) => {
    ...
    let afterFilter = Allitems.Products.filter((each) => {
      ...
      return (
        each.price <= price &&
        each.color.includes(filterState.colours) &&
        ifSizeExists
      );
    });
    setShowItems({ ...showItems, Products: afterFilter });
      ...
      <Grid container>
        {Loading ? (
          ...
          <Grid
            ...
            xl={2}
            md={2}
            sm={showFilterMenu.doSpaceGrid ? 7 : 1}
            xs={showFilterMenu.doSpaceGrid ? 7 : 1}
          >
            <h2>
              {gender} {style} {cat}
            </h2>
            ...
            <RadioGroup value={value} onChange={handleChangePrice}>
              <FormControlLabel
                value="min"
                control={<Radio />}
                label={(Allitems.minPrice + 10).toFixed(2)}
              />
              <FormControlLabel
                value="mid"
                control={<Radio />}
                label={((Allitems.maxPrice + Allitems.minPrice) / 2).toFixed(2)}
              />
              <FormControlLabel
                value="max"
                control={<Radio />}
                label={Allitems.maxPrice}
              />
              <FormControlLabel
                value="None"
                default
        ...
        <Grid
          ...
          xl={10}
          md={10}
          sm={showFilterMenu.doSpaceGrid ? 5 : 10}
          xs={showFilterMenu.doSpaceGrid ? 5 : 10}
        >
          <Container
          ...
          >
            <div className="Top">
              ...
              {isMobile ? (
                <div>
                  Filters
                  <Link>
                    <FilterListIcon fontSize="large" onClick={showFilter} />
                  </Link>
                </div>
              ) : null}
            ...
            <div align="center" className="Pagination">
              <Pagination
                ...
                totalPosts={showItems.Products.length}
                paginate={paginate}
              />
            </div>
          </Container>
