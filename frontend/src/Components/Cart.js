...
export default function Cart() {
  ...
  const SummaryAllDetails = () => {
    ...
    setTotalSummary((prevState) => {
      return { ...prevState, totalPay: totalPay, shiping: shipingPrice };
    });
  };
      ...
      <Grid direction="row" container spacing={1}>
            ...
            <Grid className="FiltersGrid" item xl={9} md={9} sm={8} xs={8}>
              <h3>
                ...
                <span style={{ color: "white" }}>
                  {totalSummary.totalPay.toFixed(2)} $
                </span>
              </h3>
              <h3>
                Shipping:
                <br />
                <span style={{ color: "white" }}>{totalSummary.shiping} $</span>
              </h3>
              <h3>
                Amount:
                <br />
              ...
              <h3>
                Total Pay :<br />
                <span style={{ color: "white" }}>
                  {(totalSummary.totalPay + totalSummary.shiping).toFixed(2)} $
                </span>
              </h3>
              {totalSummary.shiping == 0 ? null : (
                <h5>
                  You need add:
                  <br />
                  <span style={{ color: "white" }}>
        ...
        >
          <Grid container>
            <AlertMessage
              showAlert={showAlert}
              payload={"Changes Saved on your Cart"}
            />
